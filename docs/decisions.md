...
## 024 — V1'de tek user tipi: platform kullanıcısı

**Karar:** V1'de Elyzor yalnızca platform kullanıcılarını (developer / kurucu) yönetir. Son kullanıcı (alice gibi) kavramı kapsam dışıdır. Tek bir `users` koleksiyonu, tek bir `authGuard` middleware'i.

**Gerekçe:**
Elyzor'un V1 hedefi API key ve service key yönetimidir — son kullanıcı kimlik doğrulaması değil. Tek user tipi mimariyi sade tutar; `userType`, `projectGuard`, `project_users` koleksiyonu gibi ekstra katmanlar bu aşamada gereksiz karmaşıklık yaratır.

**V2 yönü:** Organizations feature ile multi-tenant rol yapısı değerlendirilebilir. O zaman `userType` claim'i ve ayrı guard'lar eklenmesi mantıklı olacak.

**Trade-off:** Platform kullanıcısı aynı zamanda son kullanıcı olan senaryolar V1'de desteklenmiyor. Bu bilinçli bir kapsam kararı.
...
- Middleware: `rateLimiter.ts`, `requestId.ts` — integration testlerinde kapsamlanır
- Error sınıfları: `errors/*.ts` — default parametre branch'leri, iş mantığı yok
...
## 031 — Stats topKeys'te keyType discriminator

**Karar:** `GET /v1/projects/:id/stats` response'undaki `topKeys` array'indeki her entry `keyType: 'api' | 'service'` field'ı taşır.

**Gerekçe:**
Usage aggregation hem `apiKeyId` hem `serviceId` bazlı kayıtları birleştirip tek `topKeys` listesi döndürür. Discriminator olmadan frontend hangi `keyId`'nin API key, hangisinin service key olduğunu bilemez — farklı endpoint'lere yönlendirme, farklı ikonlar veya filtreler için bu bilgi gereklidir.

**Uygulama:**
- `src/usage/usage.repository.ts` → `TopKey` interface'ine `keyType: 'api' | 'service'` eklendi
- API key aggregate sonuçları `keyType: 'api' as const` ile etiketlenir
- Service key aggregate sonuçları `keyType: 'service' as const` ile etiketlenir
- `src/stats/stats.types.ts` → `ProjectStatsResponse.topKeys` tipi güncellendi

**Trade-off:** Response payload'ı hafifçe büyür. Etki ihmal edilebilir — `keyType` kısa bir string literal.
